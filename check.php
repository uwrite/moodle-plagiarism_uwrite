...
/**
 * check.php - resets an uwrite submission
 *
...
require_capability('plagiarism/uwrite:checkfile', $modulecontext);

uwrite_assign::check_submitted_assignment($pf);

if ($cm->modname == 'assignment') {
    $redirect = new moodle_url('/mod/assignment/submissions.php', array('id' => $cmid));
} else {
    if ($cm->modname == UWRITE_MODNAME_ASSIGN) {
...
redirect($redirect, plagiarism_uwrite::trans('check_start'));