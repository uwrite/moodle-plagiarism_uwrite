 ...
 */
function xmldb_plagiarism_uwrite_upgrade($oldversion) {
    global $DB;

    $dbman = $DB->get_manager();

    if ($oldversion < 2016041600) {
        // Define field external_file_id to be added to plagiarism_uwrite_files.
        $table = new xmldb_table('plagiarism_uwrite_files');
        ...
        // Uwrite savepoint reached.
        upgrade_plugin_savepoint(true, 2016041600, 'plagiarism', 'uwrite');
    }
    ...
    if ($oldversion < 2016100500) {
        ...
        // Uwrite savepoint reached.
        upgrade_plugin_savepoint(true, 2016100500, 'plagiarism', 'uwrite');
    }

    if ($oldversion < 2016112200) {
        if ($dbman->table_exists('plagiarism_uwrite_user_data')) {
            $dbman->drop_table('plagiarism_uwrite_user_data');
        }

        $table = new xmldb_table('plagiarism_uwrite_user_data');
    ...
    if ($oldversion < 2017012300) {
        uwrite_core::migrate_users_access();
    }
