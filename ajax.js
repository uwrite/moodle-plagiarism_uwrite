...
M.plagiarism_uwrite.init = function (Y, contextid) {
    var handle_record = function (record) {
        ...
        if (record.progress === 100 || record.statuscode === 613 || record.statuscode === "613") {
            var items = M.plagiarism_uwrite.items;
    ...
    var track_progress = function (Y, items, contextid) {
        ...
        if (!items[0]) {
            clearInterval(M.plagiarism_uwrite.interval);
            return false;
        }

        var url = M.cfg.wwwroot + '/plagiarism/uwrite/ajax.php';
    ...
    var collect_items = function () {
        Y.all('.un_report .un_data').each(function (row) {
            ...
            M.plagiarism_uwrite.items.push(jsondata.fid);
        });
    };
    ...
    var run_plagin = function () {
        ...
        if (M.plagiarism_uwrite.items.length) {
            M.plagiarism_uwrite.interval = setInterval(function () {
                track_progress(Y, M.plagiarism_uwrite.items, contextid);
            }, 3000);
        }
    };
